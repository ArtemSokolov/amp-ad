...
synapser::synLogin()
syn <- synExtra::synDownloader( "~/data/AMP-AD/DGE1", ifcollision="overwrite.local" )
...
postQC <- function()
{
    ...
    X <- syn( "syn18143734" ) %>% read_csv( col_types = cols() )
    Y <- syn( "syn18344946" ) %>% read_csv( col_types = cols() )
